...
<style>
 body{font-family:'Inter',sans-serif;background:radial-gradient(circle at 30% 20%,#060617,#01010f 80%);color:#fff;overflow:hidden}
 h1,h2,h3{font-family:'Orbitron',sans-serif}
 .neon-text{background:linear-gradient(90deg,#00f0ff,#ff00d4);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
 .glass{background:rgba(255,255,255,.05);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.08);border-radius:1rem}
 .gradient-btn{background:linear-gradient(90deg,#00f0ff,#ff00d4);color:#000;font-weight:600;border-radius:.6rem;transition:.3s}
 .gradient-btn:hover{transform:translateY(-2px);box-shadow:0 0 20px rgba(0,255,255,.4)}
...
<section id="home" class="px-6 text-center">
 ...
 <p class="text-gray-300 mb-8 max-w-lg mx-auto">
  Explore Africa’s Web3 SuperApp — learn, earn and connect freely before you transact.
 </p>
 ...
 <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
  ...
  <div class="glass p-4 text-left md:col-span-2 hover:shadow-cyan-400/30 transition">
   ...
   <p class="text-gray-400 text-sm">
    Join the Learn & Earn program, complete mini-lessons to gain XP → convert into ZAC rewards.  
    Top students appear on leaderboards and unlock exclusive NFT badges.
   </p>
...
<section id="shop" class="hidden px-6 text-center">
 ...
 <div class="max-w-md mx-auto glass p-6 rounded-xl text-gray-300">
  <p class="mb-3">Discover student deals and campus offers soon!</p>
...
<section id="chat" class="hidden px-6 text-center">
 ...
 <div class="glass p-4 rounded-xl max-w-md mx-auto mb-4">
  ...
  <div class="flex gap-2">
   <input id="newContact" placeholder="Add new contact" class="flex-1 p-2 rounded-lg bg-[#0a0a0a] border border-gray-600 text-white">
...
// Unlock
function unlock(){alert("🔓 Connect wallet or phone to unlock full Zep Swipe access.");}

// Learn & Earn
let xp=0, lvl=1, zac=120;
document.getElementById("next").onclick=()=>{
 xp+=25;
 if(xp>=100&&lvl<5){xp=0;lvl++;zac+=15;document.getElementById("msg").textContent=`🏅 Level ${lvl} unlocked +15 ZAC`;document.getElementById("wallet").textContent=zac;}
...
function addContact(){
 ...
 if(!n)return alert("Enter a name");if(chats[n])return alert("Already added");
 chats[n]=[];const o=document.createElement("option");o.value=n;o.textContent=n;
 contactsSel.appendChild(o);document.getElementById("newContact").value="";
}
function sendMsg(){
 ...
 msg.className="self-end bg-cyan-500/20 p-2 rounded-lg mb-2 text-sm";
 msg.textContent=m;box.appendChild(msg);
 chats[c].push({from:"me",text:m});
 // auto-reply demo
 setTimeout(()=>{
  const r=document.createElement("div");
  r.className="self-start bg-pink-500/20 p-2 rounded-lg mb-2 text-sm";
...
function resize(){W=cv.width=innerWidth;H=cv.height=innerHeight;}window.onresize=resize;resize();
const orbs=Array.from({length:25},()=>({x:Math.random()*W,y:Math.random()*H,r:1+Math.random()*2,dx:(Math.random()-.5)*.7,dy:(Math.random()-.5)*.7,c:`hsl(${Math.random()*360},100%,65%)`}));
function loop(){
